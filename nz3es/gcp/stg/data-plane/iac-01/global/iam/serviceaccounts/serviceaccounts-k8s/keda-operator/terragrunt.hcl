...
locals {
  sa_name = basename(get_terragrunt_dir()) # "keda-operator"
  # Unlike the other serviceaccounts-k8s/* units, the k8s namespace here is
  # "keda" (the addons/keda umbrella chart's namespace), not "keda-operator".
  namespace = "keda"
}
...
inputs = {
  ...
  names        = [local.sa_name]
  display_name = "Workload Identity SA for ${local.sa_name} (KEDA scaler auth — Cloud Monitoring read access)"
